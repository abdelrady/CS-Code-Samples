   ...
   +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+ 
    ...
    class Program
    {
        ...
        static void RUN(object state)
        {
            ...
            //tcp = new TCPPacket(ipp, 9876, 1888, 2031, 0, 28, TCPPacket.TCPFlags.SYN, 16384, 0,
        ...
        private static void ParseIPPacket(byte[] ipPacket)
        {
            ...
            Console.Write(((char)ipPacket[20 + rcvTcpPacket.HeaderLength]).ToString());
            Console.Write(" "+((char)ipPacket[20 + rcvTcpPacket.HeaderLength + 1]).ToString() + " ");
            Console.WriteLine(((char)ipPacket[20 + rcvTcpPacket.HeaderLength + 2]).ToString());
            EndPoint ServerEP=(EndPoint) new IPEndPoint(IPAddress.Parse("127.0.0.1"),80);
            if (rcvIpPacket.Dst_Address == "192.168.2.5" && rcvIpPacket.Protocol == 0x06)
                if (rcvTcpPacket.DSTPort == 80 //&& rcvTcpPacket.SRCPort == 1888
                    //&& rcvTcpPacket.Flags == (TCPPacket.TCPFlags.ACK | TCPPacket.TCPFlags.PSH)
                    && ipPacket[20 + rcvTcpPacket.HeaderLength] == (byte)'G' && ipPacket[20+rcvTcpPacket.HeaderLength + 1] == (byte)'E'
                    && ipPacket[20+rcvTcpPacket.HeaderLength + 2] == (byte)'T')
                {
                    Console.WriteLine("HTTP GET Packet Found...");
                    ipp = new IPPacket("192.168.2.1", "192.168.2.5", 0x06);
                    tcp = new TCPPacket(ipp, rcvTcpPacket.SRCPort, 80, rcvTcpPacket.SEQNumber, rcvTcpPacket.ACKNumber, 20, TCPPacket.TCPFlags.RST, rcvTcpPacket.ReceiverWindow, 0,new byte[0], new byte[0]);

                    tcp.GenerateTCPPacket();
                    ipp.Data = tcp.Packet;
                    ipp.GenerateIPPacket();
                    MainSocket.BeginSendTo(ipp.Packet, 0, ipp.Packet.Length, SocketFlags.None,
                        ServerEP, new AsyncCallback(DataSent), MainSocket);

                    ipp = null; tcp = null;
                    ipp = new IPPacket("192.168.2.5", "192.168.2.1", 0x06);
                    tcp = new TCPPacket(ipp, 80, rcvTcpPacket.SRCPort, rcvTcpPacket.ACKNumber,(uint) (rcvTcpPacket.SEQNumber + rcvTcpPacket.Packet.Length - rcvTcpPacket.HeaderLength), 20, TCPPacket.TCPFlags.FIN | TCPPacket.TCPFlags.ACK, 16384, 0,
                        new byte[0], Encoding.Default.GetBytes("HTTP 200 OK\r\n\r\n<h1>This page is restricted</h1>"));

                    RecieverIP = new IPEndPoint(IPAddress.Parse("192.168.2.1"), rcvTcpPacket.SRCPort);
                    tcp.GenerateTCPPacket();
                    ipp.Data = tcp.Packet;
                    ipp.GenerateIPPacket();
                    MainSocket.BeginSendTo(ipp.Packet, 0, ipp.Packet.Length, SocketFlags.None,
