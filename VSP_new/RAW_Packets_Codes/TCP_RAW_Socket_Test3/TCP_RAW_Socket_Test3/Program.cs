...
namespace Change_NetworkLayer_packet_content
{
    ...
    public class TCPPacket
    {
        ...
        public void GenerateTCPPacket()
        {
            if (Data != null && (Options.Length % 4 == 0))
            {
                ...
                int EvenPaketLength=(Packet.Length%2==0)?Packet.Length:Packet.Length+1;
                byte[] tcpCheckSum = new byte[10 + EvenPaketLength];
   ...
   +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+ 
    ...
    public class IPPacket
    {
        ...
        public IPPacket(string SRC_Address, string Dst_Address, byte Protocol, byte[] ID, byte[] Flags_FragOffset, byte[] Data)
        {
            this.SRC_Address = SRC_Address;
            this.Dst_Address = Dst_Address;
            this.Protocol = Protocol;
            ...
            this.Flags_FragOffset[0] = Flags_FragOffset[0];
            this.Flags_FragOffset[1] = Flags_FragOffset[1];
            this.Data = Data;
        }
        ...
        public static UInt16 getChecksum(byte[] data, int packetsize)
        {
            ...
            while (index < packetsize)
            {
                ...
                catch (Exception exx) { Console.WriteLine("Exception: " + exx.Message + ", Packet Size: " + packetsize+" ,index: "+index); }
                index += 2;
            }
    ...
    class Program
    {
        ...
        {
            Console.Write("Enter Data To Be Sent: ");
            byte[] bytData=Encoding.Default.GetBytes(Console.ReadLine());

            IPPacket iip = new IPPacket("192.168.2.5", "192.168.2.1", 0x06, new byte[] { 0x19, 0x23 }, new byte[] { 0x40, 0x00 },new byte[0]);
            TCPPacket ttcp = new TCPPacket(iip, 1178, 1888, /*0xffffffff*/0xffff0000, 0, 29, TCPPacket.TCPFlags.PSH | TCPPacket.TCPFlags.ACK,
                17520, 0, new byte[] { 0x02, 0x04, 0x05, 0xb4, 0x01, 0x01, 0x04, 0x02 }, bytData);
            ttcp.SEQNumber = 0x5ea868fa;
            ttcp.ACKNumber = 0x0dd03f81;

            ttcp.GenerateTCPPacket();
            iip.Data = ttcp.Packet;
            iip.GenerateIPPacket();
            IPEndPoint RecieverIP = new IPEndPoint(IPAddress.Parse("192.168.2.1"), ttcp.DSTPort);
            MainSocket.SendTo(iip.Packet, (EndPoint)RecieverIP);
            Console.Write("Successfully send packet...");
        }
        ...
        private static void ParseIPPacket(byte[] ipPacket)
        {
            ...
            if (rcvIpPacket.SRC_Address == "192.168.2.5" && rcvIpPacket.Protocol == 0x06)
                if (/*rcvTcpPacket.DSTPort == 9876 &&*/ rcvTcpPacket.SRCPort == 8000
                    /*&& rcvTcpPacket.Flags == (TCPPacket.TCPFlags.ACK | TCPPacket.TCPFlags.SYN)*/)
                {
                    Console.Write("Enter Data To Be Sent: ");
                    //byte[] bytData = Encoding.Default.GetBytes(Console.ReadLine());
                    byte[] bytData = File.ReadAllBytes("4resarch_topics.txt");
                    IPPacket iip = new IPPacket("192.168.2.5", "192.168.2.1", 0x06, new byte[] { 0x19, 0x23 }, new byte[] { 0x40, 0x00 }, new byte[0]);
                    TCPPacket ttcp = new TCPPacket(iip, 8000, rcvTcpPacket.DSTPort, /*0xffffffff*/rcvTcpPacket.SEQNumber + (uint)rcvTcpPacket.Data.Length + 1, rcvTcpPacket.ACKNumber, 28, TCPPacket.TCPFlags.ACK,
                        17520, 0, new byte[] { 0x02, 0x04, 0x05, 0xb4, 0x01, 0x01, 0x04, 0x02 }, bytData);
                    //ttcp.SEQNumber = 0x5ea868fa;
                    //ttcp.ACKNumber = 0x0dd03f81;
                    ...
                    iip.Data = ttcp.Packet;
                    iip.GenerateIPPacket();
                    IPEndPoint RecieverIP = new IPEndPoint(IPAddress.Parse("192.168.2.1"), 0/*or ttcp.DSTPort*/);
                    MainSocket.SendTo(iip.Packet, (EndPoint)RecieverIP);
                    Console.Write("Successfully send packet...");
