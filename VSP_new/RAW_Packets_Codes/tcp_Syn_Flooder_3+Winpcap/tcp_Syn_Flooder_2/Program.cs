...
namespace Change_NetworkLayer_packet_content
{
    ...
    class Program
    {
        ...
        static void RUN(object state)
        {
            ...
            IPEndPoint iep = new IPEndPoint(IPAddress.Parse("192.168.3.5"), 0);//.Parse("192.168.2.5")//tcp.SRCPort
            MainSocket.Bind(iep);
            ...
            ipp = new IPPacket("192.168.3.5", "192.168.3.2",0x06);
            ...
            RecieverIP = new IPEndPoint(IPAddress.Parse("192.168.3.2"), tcp.DSTPort);
            int i = 0;
            string dstIP = "";
            string SRC_Address = "";
            for (i=1;i<255;i++)
                for (int j = 1; j < 255; j++)
                {
                    ...
                    //ipp.Protocol = 6;
                    //ipp.GenerateIPPacket();
                    SRC_Address = "192.168." + i + "." + j;
                    ipp.SRC_Address = SRC_Address;
                    
                    tcp.GenerateTCPPacket();
                    ipp.Data = tcp.Packet;

                    ipp.GenerateIPPacket();
                    //Buffer.BlockCopy(IPPacket.GetIPAddress(SRC_Address), 0, ipp.Packet, 12, 4);
