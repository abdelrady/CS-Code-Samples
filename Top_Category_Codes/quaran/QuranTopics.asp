	...
	'Ayat.ayaa=4
			for m=0 to UBound(Ayat_Result_Array,2)
				...
				if sID<>Ayat_Result_Array(0,m) Then 
					outputString="<h3 align=center >”Ê—… " & Ayat_Result_Array(2,m)  & "</h3><p>"
					ayID=0
					SorahCounter=SorahCounter+1
				END IF
				index=m+1
				SorahLink="(<font size=2><A href=""quran.asp?page=" & Ayat_Result_Array(3,m)  & """>" & Ayat_Result_Array(2,m) & "</A></font>)"
				...
				if index<=UBound(Ayat_Result_Array,2) Then
					if Clng(Ayat_Result_Array(1,index))-Clng(Ayat_Result_Array(1,m))<>1 Then 
						outputString=outputString & SorahLink & "<BR>"
					Else
						outputString=outputString &  " "
					End if
				Else
					outputString=outputString & SorahLink & "</p>"
				End if
