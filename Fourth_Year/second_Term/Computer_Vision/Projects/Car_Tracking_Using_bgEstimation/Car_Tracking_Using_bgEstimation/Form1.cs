...
namespace Car_Tracking_Using_bgEstimation
{
    public partial class Form1 : Form
    {
        ...
        public static extern void mciSendString(string msg,ref string ErrBuf, int length, IntPtr handle);
        ...
        class FramingThread
        {
            ...
            void Scan()
            {
                ...
                //int x = 0;
                for (float i = 0.0f; i < len/2; i = i + 0.1f)
                {
                    counter++;
                    if (counter > 250) break;
                    string fBitmapName = storagePath + counter.ToString();

                    //md.GetBitmapBits(i,ref buffSize,
                    md.WriteBitmapBits(i, 150, 150, fBitmapName + ".jpg");
            ...
            for (int i = 1; i <= num;i++ )
                ...
                for (int x = 0; x < 150; x++)
                {
                    ...
                    for (int y = 0; y < 150; y++)
                    {
                        ...
                        grayScale = (int)(avgRed * 0.299 + avgGreen * 0.587 + avgBlue * 0.114);//y-cb-cr
        ...
        private void miOpenFile_Click(object sender, EventArgs e)
        {
            ...
            ofdlg.Filter = "AVI Video Files | *.avi| All Files | *.*";     //to select avi files only
            ...
            if (ofdlg.ShowDialog() == DialogResult.OK)
            {
                ...
                try
                {
                    ...
                    mciSendString("OPEN \"" + fileName + "\" TYPE MPEGVideo ALIAS InputVideo PARENT " 
                        + InputVideoPanel.Handle + " STYLE child",ref errBuf, 0, IntPtr.Zero);
                    ...
                    mciSendString("PUT InputVideo WINDOW AT " + (0).ToString() + " "
                        ...
                        + " " + (InputVideoPanel.Height).ToString(),ref errBuf, 0, IntPtr.Zero);

                    mciSendString("PLAY InputVideo",ref errBuf, 0, IntPtr.Zero);   //REPEAT
        ...
        private void miThreshold_Click(object sender, EventArgs e)
        {
            ...
            mciSendString("open \"" + Application.StartupPath + "\\Thresholded.avi" 
                ...
                + ThresholdedVideopanel.Handle + " style child",ref errBuf, 0, IntPtr.Zero);

            //to make the video stretched to the showing window
            mciSendString("PUT ThresholdedVideo WINDOW AT " + (0).ToString() + " "
                + (0).ToString() + " " + (ThresholdedVideopanel.Width).ToString()
                + " " + (ThresholdedVideopanel.Height).ToString(),ref errBuf, 0, IntPtr.Zero);

            mciSendString("play ThresholdedVideo REPEAT",ref errBuf, 0, IntPtr.Zero);
        ...
        private void miGenerateOutVideo_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                mciSendString("open \"" + Application.StartupPath + "\\Out.avi"
                    ...
                    + OutputVideoPanel.Handle + " style child",ref errBuf, 0, IntPtr.Zero);

                //to make the video stretched to the showing window
                mciSendString("PUT OutVideo WINDOW AT " + (0).ToString() + " "
                    + (0).ToString() + " " + (OutputVideoPanel.Width).ToString()
                    + " " + (OutputVideoPanel.Height).ToString(),ref errBuf, 0, IntPtr.Zero);

                mciSendString("play OutVideo REPEAT",ref errBuf, 0, IntPtr.Zero);
