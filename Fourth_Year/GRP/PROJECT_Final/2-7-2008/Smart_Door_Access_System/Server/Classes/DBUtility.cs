...
namespace SmartDoor
{
    ...
    public class DBUtility
    {
        ...
        public SqlConnection m_DbConnection;
        ...
        public User_Status GetRow(string mDoorNumber, string Barcode, out string[] out_Data, out byte[] Image_Data)
        {
            ...
            try
            {
                ...
                userStatusTableAdapter.Connection.ConnectionString = Configuration.connectionString;
                int? status = 0;
                DataSet1.UserStatusDataTable userStatusDataTable = userStatusTableAdapter.GetData(Barcode, int.Parse(mDoorNumber), ref  status);
                if (userStatusDataTable.Rows.Count > 0)
                {
                    ...
                    if (userStatusDataTable.Rows[0].IsNull("UserImage") ==false)
                    {
                        Image_Data = ImageCompressor.Compress((byte[])userStatusDataTable.Rows[0]["UserImage"], ImageQuality);
            ...
            {
                MessageBox.Show(mParent, "Can not Get Row,\n\r " + d.Message, "Error accessing database", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return User_Status.None;
            }
            return Us;
        }
        ...
        internal void Synchronize_Door_Name(string Door_Name, string Door_Num,string IP)
        {

            SqlCommand cmd = new SqlCommand();

            cmd.Connection = m_DbConnection;
            cmd.CommandText = "UPDATE Doors SET doorName='" + Door_Name + "', DoorIP='" + IP + "' WHERE doorNum=" + Door_Num;
                cmd.ExecuteNonQuery();
        ...
        internal void Fill(DataGridView dataGridView1,string Query)
        {
            dataGridView1.Refresh();
            SqlCommand sqlCmd = new SqlCommand(Query, m_DbConnection);
        ...
        internal object GetDataColumn(string p)
        {
            ArrayList values = new ArrayList();
            if(p=="Door Names")
            {
                SqlCommand cmd = new SqlCommand("SELECT distinct Doors.doorName FROM    Logs INNER JOIN  Doors ON Logs.doorNumber = Doors.doorNum ", m_DbConnection);
                SqlDataReader dr = cmd.ExecuteReader();
                //SqlDataReader dr = cmd.ExecuteReader();
                if (dr.HasRows)
                {
            ...
            {
                SqlCommand cmd = new SqlCommand("SELECT distinct Users.userfName + ' ' + Users.userlName FROM    Logs INNER JOIN  Doors ON Logs.doorNumber = Doors.doorNum INNER JOIN Users ON Logs.ID = Users.userID", m_DbConnection);
