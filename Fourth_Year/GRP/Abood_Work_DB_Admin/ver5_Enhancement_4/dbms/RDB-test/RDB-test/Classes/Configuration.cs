    ...
    class Configuration
    {
        ...
        private static void SetConfigurationAttributes(string[] p)
        {
            try
            {
                ...
                ConnectionTimeout = p[3].Split('=')[1];
                UserInstance = p[4].Split('=')[1];
                userName = p[5].Split('=')[1];
                userPassword = p[6].Split('=')[1];
            }
            catch { }
        }
        ...
        internal static bool Valid()
        {
            ...
            catch
            {
                ...
                Write_Config(@"Data Source=.\SQLEXPRESS;AttachDbFilename='C:\db\SDASDB.mdf';Integrated Security=True;Connect Timeout=30;User Instance=True;User ID=;Password=");
                return false;
            }
            if (connectionString == "")
            {
        ...
        internal static void Write_Config(string userName, string userPassword, string ServerName, string DBName, string IntegratedSecurity, string ConnectionTimeout, string UserInstance)
        {
            ...
            Writer.WriteLine("connectionString=" + "Data Source=" + Configuration.ServerName + ';' + "Initial Catalog=" + Configuration.DBName + ';'
                + "Integrated Security=" + Configuration.IntegratedSecurity + ';' + "Connect Timeout=" + Configuration.ConnectionTimeout + ';' + "User Instance=" + Configuration.UserInstance + ';' + "User ID=" + Configuration.userName + ';' +
                "Password=" + Configuration.userPassword);
                //Configuration.Encrypt);//';'+Configuration.persistSecInfo+
            Writer.Close();    
        }
