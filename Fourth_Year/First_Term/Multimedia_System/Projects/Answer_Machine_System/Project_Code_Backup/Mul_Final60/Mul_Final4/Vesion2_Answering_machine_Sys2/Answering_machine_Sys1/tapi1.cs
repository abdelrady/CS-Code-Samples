...
namespace tapi1_cs
{
    public class TAPI
    {
        ...
        public int hLine;
        public linedevcaps lpLineDevCaps;
        public Form1 userInterface;
        public int TAPIVERSION = 0x10004;
        public short LINECALLPRIVILEGE_OWNER = 0x4;
        public short LINECALLPRIVILEGE_MONITOR = 0x2;
        public short LINEMEDIAMODE_AUTOMATEDVOICE = 0x8;
        ...
        public short LINEDIGITMODE_DTMF = 0x2;
        public const short LINE_MONITORDIGITS = 9;
        public int LINECALLSELECT_CALL = 0x4;
        public const int LINEDEVSTATE_RINGING = 0x00000002;

        public delegate void ShowMessage(string msg);
        public event ShowMessage ShowThisMessage;

        public delegate void AcceptCall();
        public event AcceptCall CallAccepted;

        public delegate void DropCall();
        public event DropCall CallDropped;

        public delegate void PlayWaveFile(string File);
        public event PlayWaveFile OnPlayWaveFile;
        
        public delegate void add2DB(int Rate);
        public event add2DB Add2DB;
    ...
    public int LineCallBack(int dwDevice, int dwMessage,
            int dwInstance, int dwParam1, int dwParam2, int dwParam3)
        {
            ...
            switch (dwMessage)
            {
                ...
                case LINE_MONITORDIGITS:
                    msgEvent = "DTMF";
                    if (!firstDigit )//&& !audio.audio.playing)
                    {
                        if (dwParam1 > 48 && dwParam1 < 52)
                        {
                            firstDigit = true;
                            Add2DB(dwParam1 - 48);
                        ...
                        else
                        {
                            ...
                            OnPlayWaveFile("Bad_choise.wav");
                        }
                        
                    }
                    if (dwParam1 > 47)
                    {
                        ShowThisMessage("System User Press "
                ...
                case LINE_LINEDEVSTATE:
                    switch (dwParam1)
                    {
                        ...
                        //How many rings we answer on, dwParam3 contain Number of rings now.
                            msgEvent = "Ringing";
                            if (dwParam3 > RingsCount)
                                CallAccepted();
                        break;
                    }
                    break;
            }
            ShowThisMessage("Event Occured: " + msgEvent + " Data:" + dwParam1 );
            return 1;
        }

        private void getInfo(int hcall)
        {
            int nError = 0;
            ...
            if (nError != 0)
            {
                ShowThisMessage("[" + lineinfoobj.bBytes + "]");
            }
            else
            {
                ShowThisMessage("lineCallInfo error.....");
            }
        }
        /*
        ...
        public string GetLineID(string sWave)
        {
            //return "1";
            long nError = 0;
            string sTemp = "";
            ...
            //ShowThisMessage("["+oVar.bBytes+"]");
            if (oVar.dwStringOffset == 0)
            {
                ShowThisMessage(nError.ToString());
                return "-1";
            }
            ...
            //ShowThisMessage("stemp:="+sTemp2);
            //ShowThisMessage("id:=" + id.ToString());
            //ShowThisMessage("oVar.dwStringOffset =" + rawdata[20].ToString());
            //ShowThisMessage("oVar.dwStringOffset =" + rawdata[21].ToString());
            //ShowThisMessage("oVar.dwStringOffset =" + rawdata[22].ToString());
            //ShowThisMessage("oVar.dwStringOffset =" + rawdata[23].ToString());
            //ShowThisMessage("oVar.dwStringSize =" + rawdata[16].ToString());
            //ShowThisMessage("oVar.dwStringSize =" + rawdata[17].ToString());
            //ShowThisMessage("oVar.dwStringSize =" + rawdata[18].ToString());
            //ShowThisMessage("oVar.dwStringSize =" + rawdata[19].ToString());

            //ShowThisMessage("oVar.lineID =" + rawdata[24].ToString());
            //ShowThisMessage("oVar.lineID =" + rawdata[25].ToString());
            //ShowThisMessage("oVar.lineID =" + rawdata[26].ToString());
            //ShowThisMessage("oVar.lineID =" + rawdata[27].ToString());
            //if (oVar.dwStringOffset == 0)
            //{
                //ShowThisMessage("oVar.dwStringOffset =" + rawdata[ oVar.dwStringSize.ToString());
              //  return "-1";
            //}
            //sTemp = rawdata[24].ToString();
            //byte[] byt =Form1.RawSerialize(oVar);
            //sTemp = rawdata[oVar.dwStringOffset].ToString();
            //ShowThisMessage("line id="+ sTemp);
            //for (int i = 27; i >=24; i--)
            //{
           //     sTemp2 += (rawdata[i]).ToString();
                //ShowThisMessage( rawdata[i].ToString());//(char)
            //}
